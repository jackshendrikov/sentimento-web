...
def delete_task(request, task_id: int) -> HttpResponse:
    """Delete specified task. Redirect to the list from which the task came."""

    if request.method == "POST":
        task = get_object_or_404(Task, pk=task_id)

        redir_url = reverse("analyzer:list_detail",
                            kwargs={"list_id": task.task_list.id, "list_slug": task.task_list.slug}, )

        # Permissions
        if not ((task.created_by == request.user) or request.user.is_superuser or (task.assigned_to == request.user)
